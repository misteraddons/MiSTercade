...
2. Run the installer on MiSTer.
   - Shows the MiSTercade mappings already configured on your SD card before you change anything.
   - Runs `lsusb` to detect MiSTercade V2 by `16D0:1358` and MiSTercade V1-family hardware by `16D0:10BE`.
   - Defaults `16D0:10BE` hardware to MiSTercade V1 2025 because legacy V1 uses the same USB ID and is deprecated.
   - Checks for MiSTercade sections in `/media/fat/downloader.ini` and can back up that file before removing those sections.
   - Checks for existing MiSTercade `downloader_*.ini` files in `/media/fat` and removes conflicting choices for the MiSTercade version you select.
   - Checks for existing `/media/fat/config/inputs/*_VID_PID_v3.map` files matching your selected MiSTercade version and can create a timestamped backup archive before you continue.
   - Checks for `/media/fat/gamecontrollerdb/gamecontrollerdb_user.txt` and can back it up and remove it when you switch back to `.map` files.
   - Can back up and update `/media/fat/MiSTer.ini` with MiSTercade-friendly settings. The shared V1/V2 video settings are `direct_video=2`, `composite_sync=1`, and `vga_mode=rgb`.
   - Downloads the selected `downloader_[configuration name].ini` file to `/media/fat`.
3. Confirm the detected MiSTercade version, or choose manually if auto-detection is unavailable.
...
8. Run your preferred MiSTer updater script.
